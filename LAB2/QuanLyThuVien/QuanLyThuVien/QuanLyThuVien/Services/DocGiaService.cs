...
namespace QuanLyThuVien.Services
{
    public class DocGiaService
    {
        ...
        {
            return Db.Query(@"SELECT dg.MaDocGia, dg.Ho, dg.Ten, dg.NgaySinh, dg.Phai, dg.SoDienThoai, dg.DiaChi, dg.Email, dg.Anh3x4, t.MaThe, t.NgayCap, t.HanSuDung, t.DaDongLePhi, t.TrangThai 
                              FROM DocGia dg 
                              OUTER APPLY (SELECT TOP 1 * FROM TheDocGia x WHERE x.MaDocGia=dg.MaDocGia ORDER BY x.TrangThai DESC, x.HanSuDung DESC) t 
        ...
        {
            if (dg == null || string.IsNullOrWhiteSpace(dg.MaDocGia) || string.IsNullOrWhiteSpace(dg.Ho) || string.IsNullOrWhiteSpace(dg.Ten) || string.IsNullOrWhiteSpace(dg.Phai) || string.IsNullOrWhiteSpace(dg.DiaChi) || string.IsNullOrWhiteSpace(dg.Email))
            ...
            {
                string sql = capNhat
                    ? @"UPDATE DocGia SET Ho=@Ho,Ten=@Ten,NgaySinh=@NgaySinh,Phai=@Phai,SoDienThoai=@SDT,DiaChi=@DiaChi,Email=@Email,Anh3x4=@Anh WHERE MaDocGia=@Ma"
                    : @"INSERT INTO DocGia(MaDocGia,Ho,Ten,NgaySinh,Phai,SoDienThoai,DiaChi,Email,Anh3x4) VALUES(@Ma,@Ho,@Ten,@NgaySinh,@Phai,@SDT,@DiaChi,@Email,@Anh)";
                ...
                return n > 0 ? KetQuaXuLy.Ok(capNhat ? "Cập nhật độc giả thành công." : "Thêm độc giả thành công.") : KetQuaXuLy.Loi("Không có dữ liệu được thay đổi.");
            }
        ...
        public KetQuaXuLy CapThe(string maDocGia, DateTime ngayCap, DateTime hanSuDung, bool daDongLePhi)
        {
            ...
            using (SqlTransaction tx = cn.BeginTransaction())
            {
                try
                {
                    using (SqlCommand check = new SqlCommand(@"SELECT COUNT(*) FROM TheDocGia WHERE MaDocGia=@Ma AND TrangThai=1 AND HanSuDung>=@NgayCap", cn, tx))
                    {
        ...
        {
            using (SqlConnection cn = Db.OpenConnection())
            using (SqlCommand cmd = new SqlCommand(@"SELECT TOP 1 MaThe,NgayCap,HanSuDung FROM TheDocGia WHERE MaDocGia=@Ma AND TrangThai=1 ORDER BY HanSuDung DESC", cn))
            {
