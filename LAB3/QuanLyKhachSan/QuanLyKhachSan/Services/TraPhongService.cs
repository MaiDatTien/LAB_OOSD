    ...
    public class TraPhongService
    {
        ...
        public KetQuaXuLy LapHoaDon(string soHD, string soDat, DateTime ngay, string maNV, int soNgay)
        {
            ...
            try
            {
                ...
                // ĐÃ SỬA: Loại bỏ cột TrangThai khỏi câu lệnh INSERT để phù hợp với database của bạn
                Db.Execute("INSERT INTO HoaDon(SoHoaDon, SoPhieuDat, NgayLap, MaNV, SoNgayTinhTien, TienPhong, TienDichVu) VALUES(@h,@s,@n,@nv,@ng,@p,@d)",
        ...
        public KetQuaXuLy ThanhToan(string maTT, string soHD, DateTime ngay, string hinhThuc, decimal tien)
        {
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    ...
                    // ĐÃ SỬA: Không UPDATE TrangThai nữa vì database của bạn không có cột này
        ...
        public KetQuaXuLy TraPhong(string soDat, DateTime ngayTra)
        {
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    // ĐÃ SỬA: Gom nhóm tính toán kiểm tra Tổng tiền hóa đơn so với Tổng tiền đã thanh toán
                    var q = new SqlCommand(@"
                        SELECT h.SoHoaDon, h.TongTien, ISNULL(SUM(t.SoTien), 0) AS DaTra
                        FROM HoaDon h 
                        LEFT JOIN ThanhToan t ON h.SoHoaDon = t.SoHoaDon 
                        WHERE h.SoPhieuDat=@s
                        GROUP BY h.SoHoaDon, h.TongTien", cn, tx);

                    q.Parameters.AddWithValue("@s", soDat);

                    decimal tongTien = 0, daTra = 0;
                    bool coHoaDon = false;
                    ...
                    using (var rd = q.ExecuteReader())
                    {
                        if (rd.Read())
                        {
                            ...
                            daTra = Convert.ToDecimal(rd["DaTra"]);
                        }
                    }

                    if (!coHoaDon) return KetQuaXuLy.Fail("Chưa lập hóa đơn cho phiếu đặt phòng.");


                    if (daTra < tongTien) return KetQuaXuLy.Fail("Hóa đơn chưa thanh toán đủ.");
